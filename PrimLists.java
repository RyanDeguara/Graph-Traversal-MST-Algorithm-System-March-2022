...
{ // start GraphLists class
	...
	{ //start MST_Prim operation
        ...
        //code here
        /*
        Loop goes through all vertices, dist marked to 
        infinity,  hPos = 0 – not on the heap, when create the 
        heap tell it the number of vertices, give the heap 
        access to hPos array, mark v distance as negative, for 
        loop looks at each vertex u that is connected to v, if 
        weight of connection is less than distance to u update 
        the distance, if u is not on the heap can check this by 
        hPos, insert into heap. While loop goes through every 
        edge on the graph
        */
...
// and nodes are removed from the front therefore use of pointers of head and tail
